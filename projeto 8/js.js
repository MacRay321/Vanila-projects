...
let contador = 0;
    ...
    for(let i = 0; i<NOMES.length; i++){
        ...
        if(NOMES[i].innerText.indexOf((INPUT.value).toUpperCase()) >= 0) D[i].style.display = "block";
        else D[i].style.display = "none"

    D.forEach((x)=>{
        
        if(x.style.display == "none") contador++;

    })

          
    if(a.parentElement == UL) a.parentElement.removeChild(a);
    
    if(contador == D.length) UL.appendChild(a);
    else contador = 0;
...
SUBMIT.addEventListener("click",()=>{
    ...
    fetch(`https://www.mercadobitcoin.net/api/${valor}/ticker/`)
    .then((x)=> x.json())
    .then((g)=>{
        ...
        span.innerHTML = informacoes;

        UL.appendChild(div);

    })
    .catch((c)=> console.log("não encontrado"));
